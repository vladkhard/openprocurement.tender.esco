...
def create_tender_lot_invalid(self):
    response = self.app.post_json('/tenders/some_id/lots',
                                  {'data': {'title': 'lot title',
                                            'description': 'lot description'}}, status=404)
    ...
    response = self.app.patch_json('/tenders/{}?acc_token={}'.format(
        self.tender_id, self.tender_token), {"data": {
            "items": [{'relatedLot': '0' * 32}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
def patch_tender_lot_minValue(self):
    ...
    response = self.app.patch_json('/tenders/{}/lots/{}?acc_token={}'.format(
        self.tender_id, lot['id'], self.tender_token), {"data": {"minValue": {"amount": 300}}})
...
def get_tender_lot(self):
    ...
    self.assertEqual(response.json['errors'], [
        {u'description': u'Not Found',
         u'location': u'url', u'name': u'lot_id'}
    ])
    ...
    self.assertEqual(response.json['errors'], [
        {u'description': u'Not Found',
         u'location': u'url', u'name': u'tender_id'}
    ])
...
def lot_minimal_step_invalid(self):
    ...
    response = self.app.patch_json('/tenders/{}/lots/{}?acc_token={}'.format(
        self.tender_id, lot['id'], self.tender_token), {"data": {"minimalStep": {"amount": 300}}})
    self.assertEqual(response.status, '200 OK')
    self.assertNotIn('minimalStep', response.json['data'])
...
def tender_minimal_step_percentage(self):
    ...
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['data']['minimalStepPercentage'],
                     self.test_lots_data[0]['minimalStepPercentage'])
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['data']['minimalStepPercentage'],
                     min([i['minimalStepPercentage'] for i in self.test_lots_data]))
...
def tender_lot_funding_kind(self):
    ...
    response = self.app.post_json('/tenders/{}/lots?acc_token={}'.format(
        tender['id'], tender_token), {'data': lot})
    self.assertEqual(response.status, '201 Created')
    self.assertIn('fundingKind', response.json['data'])
    self.assertEqual(response.json['data']['fundingKind'], 'budget')
    ...
    lot['fundingKind'] = 'budget'
    response = self.app.post_json('/tenders/{}/lots?acc_token={}'.format(
        tender['id'], tender_token), {'data': lot}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/{}/lots/{}?acc_token={}'.format(
        tender['id'], lot1_id, tender_token), {'data': lot})
    self.assertEqual(response.status, '200 OK')
    self.assertIn('fundingKind', response.json['data'])
    self.assertEqual(response.json['data']['fundingKind'], 'other')
...
def tender_lot_yearlyPaymentsPercentageRange(self):
    ...
    self.assertEqual(response.json['data']['yearlyPaymentsPercentageRange'],
                     min([i['yearlyPaymentsPercentageRange'] for i in lots]))

    response = self.app.patch_json('/tenders/{}?acc_token={}'.format(tender_id, tender_token),
    ...
    response = self.app.patch_json('/tenders/{}/lots/{}?acc_token={}'.format(
        tender_id, lot1_id, tender_token), {'data': {'yearlyPaymentsPercentageRange': 0.3}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.patch_json('/tenders/{}/lots/{}?acc_token={}'.format(
        tender_id, lot1_id, tender_token), {'data': {
            'yearlyPaymentsPercentageRange': 0.9}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/{}?acc_token={}'.format(
        tender_id, tender_token), {'data': {
            'fundingKind': 'other', 'yearlyPaymentsPercentageRange': 0.8}})
    self.assertEqual(response.status, '200 OK')
    self.assertIn('fundingKind', response.json['data'])
    self.assertEqual(response.json['data']['fundingKind'], 'other')
    self.assertEqual(response.json['data']['yearlyPaymentsPercentageRange'], 0.8)
...
def tender_min_value(self):
    ...
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['data']['minValue']['amount'],
                     sum([i['minValue']['amount'] for i in self.initial_lots]))
...
def create_tender_bid_invalid(self):
    ...
    response = self.app.post_json(request_path, {'data': {
        ...
        'lotValues': [{"value": self.test_bids_data[0]['value']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {
        'selfEligible': True, 'selfQualified': True,
        'tenderers': self.test_bids_data[0]['tenderers'],
        'lotValues': [{"value": {'yearlyPaymentsPercentage': 0.9,
                                 'annualCostsReduction': [751.5] * 11,
                                 'contractDuration': {'years': 10}},
                       'relatedLot': self.initial_lots[0]['id']}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
        {u'description': [{u'value':{u'annualCostsReduction': [u'annual costs reduction should be set for 21 period']}}],
         u'location': u'body', u'name': u'lotValues'}
    ])
    ...
    response = self.app.post_json(request_path, {'data': {
        'selfEligible': True, 'selfQualified': True,
        'tenderers': self.test_bids_data[0]['tenderers'],
        'lotValues': [{'value': {'yearlyPaymentsPercentage': 0.8,
    ...
    self.assertEqual(response.json['errors'], [
        {u'location': u'body', u'name': u'lotValues',
         u'description': [{u'value': [u'valueAddedTaxIncluded of bid should be identical to valueAddedTaxIncluded of minValue of lot']}]}
    ])
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
        {u'description': [u'value should be posted for each lot of bid'],
...
def patch_tender_bid(self):
    ...
    self.assertEqual(response.status, '201 Created')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['data']['lotValues'][0]['value']['amount'],
                     self.expected_bid_amount)
    self.assertEqual(response.json['data']['lotValues'][0]['value']['amountPerformance'],
                     self.expected_bid_amountPerformance)
    bid = response.json['data']
    bid_token = response.json['access']['token']

    response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(
        self.tender_id, bid['id'], bid_token), {"data": {
            'tenderers': [{"name": u"Державне управління управлінням справами"}]}})
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    ...
    response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(
        self.tender_id, bid['id'], bid_token), {'data': {
            'lotValues': [{'value': {'currency': 'USD'}}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.patch_json('/tenders/{}/bids/{}?acc_token={}'.format(
        self.tender_id, bid['id'], bid_token), {'data': {
            'lotValues': [{'value': {'valueAddedTaxIncluded': False}}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    self.assertEqual(response.status, '200 OK')
    self.assertEqual(response.content_type, 'application/json')
    self.assertNotEqual(response.json['data']['lotValues'][0]['value']['amount'],
                        self.expected_bid_amount)
    self.assertNotEqual(response.json['data']['lotValues'][0]['value']['amountPerformance'],
                        self.expected_bid_amountPerformance)
...
def create_tender_feature_bid_invalid(self):
    ...
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
        {u'description': [u'All features parameters is required.'],
         u'location': u'body', u'name': u'parameters'}
    ])
    ...
    response = self.app.post_json(request_path, {'data': {
        ...
        'parameters': [{"code": "code_item", "value": 0.01}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {
        ...
        'lotValues': [{"value": self.test_bids_data[0]['value'],
                       'relatedLot': self.lot_id}],
        'parameters': [{"code": "code_invalid", "value": 0.01}]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
    ...
    response = self.app.post_json(request_path, {'data': {
        ...
        'parameters': [
            ...
            {"code": "code_lot", "value": 0.01},]}}, status=422)
    self.assertEqual(response.status, '422 Unprocessable Entity')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['status'], 'error')
    self.assertEqual(response.json['errors'], [
...
def create_tender_feature_bid(self):
    ...
    response = self.app.post_json(request_path, {'data': {
        ...
        'parameters': [
            {"code": "code_item", "value": 0.01},
            {"code": "code_tenderer", "value": 0.01},
            {"code": "code_lot", "value": 0.01},]}}, status=403)
    self.assertEqual(response.status, '403 Forbidden')
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(response.json['errors'][0]["description"],
                     "Can't add bid in current (unsuccessful) tender status")
...
def tender_features_invalid(self):
    ...
    self.assertEqual(response.json['errors'], [
        {u'description':
         [u'Sum of max value of all features for lot should be less then or equal to {0:.0%}'.format(self.sum_of_max_value_of_all_features)],
         u'location': u'body', u'name': u'features'}
    ])
